...
import { getAdminVoters, uploadVotersCSV, getVoterUploads, OrgMember, VoterUploadResult } from '../api/elections'
...
export default function AdminVotersPage() {
  ...
  const load = async () => {
    try {
      ...
      flash('Failed to load voters.', true)
    } finally {
      setLoading(false)
    }
  }
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] gap-4">
        ...
        <p className="text-sm" style={{ color: 'var(--muted)' }}>Loading voters…</p>
      </div>
    )
  }
      ...
      {tab === 'list' && (
        <div className="space-y-4">
          <div className="fade-up-1">
            <input className="input-field" placeholder="Search by username, email, voter ID, name…"
                    ...
                    {filteredVoters.map((v, i) => (
                      <tr key={v.id}
                        style={{
                          ...
                          background: i % 2 === 0 ? 'transparent' : 'color-mix(in srgb, var(--bg-light) 20%, transparent)',
                        }}>
                        ...
                        <td className="px-4 py-3">
                          <span className="text-xs px-2 py-0.5 rounded-sm font-semibold"
                            style={{
                              color: v.role === 'ADMIN' ? 'var(--accent)' : 'var(--muted)',
                              background: v.role === 'ADMIN' ? 'color-mix(in srgb, var(--accent) 10%, transparent)' : 'color-mix(in srgb, var(--muted) 10%, transparent)',
        ...
        <div className="space-y-6">
          ...
          <div className="glass-card p-6 fade-up-1">
            ...
            <p className="text-sm mb-4" style={{ color: 'var(--muted)' }}>
              Your CSV file must include these columns. Use the template below.
            </p>
            ...
            <div className="overflow-x-auto mb-4">
              <table className="text-xs w-full border-collapse">
                ...
                <tbody>
                  {[
                    ['jdoe', 'jdoe@school.edu', 'SecurePass1!', 'STU-001', 'John Doe'],
                    ['asmith', 'asmith@school.edu', 'SecurePass2!', 'STU-002', 'Alice Smith'],
                  ].map((row, i) => (
                    <tr key={i}>
                      {row.map((cell, j) => (
                        <td key={j} className="px-3 py-2"
                          style={{ color: 'var(--muted)', border: '1px solid color-mix(in srgb, var(--card-border) 50%, transparent)' }}>
                          {cell}
                        </td>
                      ))}
                    </tr>
          ...
          <div className="glass-card p-6 fade-up-2">
            ...
            <label
              className="flex flex-col items-center justify-center gap-3 p-10 rounded-sm cursor-pointer transition-all"
              style={{
              ...
              <p className="text-sm font-semibold" style={{ color: 'var(--cream)' }}>
                {uploading ? 'Uploading…' : 'Click to select or drag & drop your CSV'}
              </p>
            ...
            <div className="glass-card p-6 fade-up space-y-4">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="text-center p-3 rounded-sm"
                  style={{ background: 'color-mix(in srgb, var(--muted) 8%, transparent)', border: '1px solid var(--card-border)' }}>
                ...
                <div>
                  ...
                  <div className="max-h-40 overflow-y-auto space-y-1 pr-1">
                    {uploadResult.errors.map((e, i) => (
                      <p key={i} className="text-xs px-3 py-1.5 rounded-sm"
                        style={{ background: 'color-mix(in srgb, var(--danger) 6%, transparent)', color: 'var(--danger)' }}>
                        {e}
                      </p>
                    ))}
                  </div>
