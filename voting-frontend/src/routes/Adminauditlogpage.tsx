...
import { getAuditLog, AuditLog } from '../api/elections'

const ACTION_COLORS: Record<string, string> = {
  LOGIN: 'var(--success)',
  LOGOUT: 'var(--muted)',
  VOTE_CAST: 'var(--accent)',
  ELECTION_CREATED: '#6366f1',
  ELECTION_UPDATED: '#6366f1',
  ELECTION_DELETED: 'var(--danger)',
  CANDIDATE_ADDED: '#f59e0b',
  CANDIDATE_UPDATED: '#f59e0b',
  CANDIDATE_DELETED: 'var(--danger)',
  VOTER_UPLOADED: '#ec4899',
  MEMBER_ROLE_CHANGED: '#60a5fa',
  MEMBER_REMOVED: 'var(--danger)',
  JOIN_CODE_REGENERATED: '#f59e0b',
  ORG_SETTINGS_UPDATED: '#60a5fa',
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] gap-4">
        ...
        <p className="text-sm" style={{ color: 'var(--muted)' }}>Loading audit log…</p>
      </div>
    )
  }
    ...
    <div className="max-w-6xl mx-auto px-4 py-10 space-y-6">
      <div className="fade-up">
        ...
        <p className="text-sm mt-1" style={{ color: 'var(--muted)' }}>
          {logs.length} events recorded · Showing last 200
        </p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 fade-up-1">
        <input className="input-field flex-1" placeholder="Search by user, detail, IP address…"
      ...
      <div className="flex flex-wrap gap-3 fade-up-2">
        {[
          ...
          <div key={label} className="px-4 py-2 rounded-sm flex items-center gap-2"
                    ...
                    <div className="flex items-center gap-2 flex-wrap mb-0.5">
                      ...
                      {log.username && (
                        <span className="text-xs px-2 py-0.5 rounded-sm"
                          style={{ background: 'color-mix(in srgb, var(--muted) 10%, transparent)', color: 'var(--muted)' }}>
                          {log.username}
                        </span>
                      )}
                    </div>
                    {log.detail && (
                      <p className="text-sm" style={{ color: 'var(--muted)' }}>{log.detail}</p>
                    )}
